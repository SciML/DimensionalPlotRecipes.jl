...
@recipe function f(
        x::AbstractArray{T}, y::Array{Complex{T2}};
        transformation = :split3D
    ) where {T <: Real, T2 <: Number}
    if transformation == :split2D
        A = real.(y)
        B = imag.(y)
        _y = [!iseven(i) ? A[:, i ÷ 2 + 1] : B[:, i ÷ 2] for i in 1:2size(A, 2)]
        linestyle -->
        reshape([!iseven(i) ? :solid : :dash for i in 1:2size(A, 2)], 1, 2size(A, 2))
        label -->
        reshape([!iseven(i) ? "Re(y$(i ÷ 2 + 1))" : "Im(y$(i ÷ 2))" for i in 1:2size(A, 2)], 1, 2size(A, 2))
        seriescolor -->
        reshape([!iseven(i) ? i ÷ 2 + 1 : i ÷ 2 for i in 1:2size(A, 2)], 1, 2size(A, 2))
        retval = (x, _y)
